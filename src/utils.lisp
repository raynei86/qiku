...
;; Piece related utils
(declaim (ftype (function (position mailbox-index) piece) piece-at)
	 (inline piece-at))
(defun piece-at (position square)
  (aref (position-mailbox position) square))
...
;; Board related utils
(declaim (ftype (function (position mailbox-index) boolean) square-occupied-p)    (inline square-occupied-p))
(defun square-occupied-p (position square)
  (/= +empty+ (piece-at position square)))

(declaim (ftype (function (position mailbox-index color) boolean) square-occupied-by-p)
	 (inline square-occupied-by-p))
(defun square-occupied-by-p (position square color)
  (let ((piece (piece-at position square)))
    (and (not (zerop piece))
	 (= color (piece-color (piece-at position square))))))
...
(declaim (ftype (function (position color piece) (unsigned-byte 64)) enemy-bb))
(defun enemy-bb (position friendly-color bb-type)
  (let ((enemy-color (enemy-of friendly-color)))
    (if (= enemy-color +white+)
	(case bb-type
	  (#.+pawn+ (position-white-pawns position))
	  (#.+rook+ (position-white-rooks position))
	  (#.+knight+ (position-white-knights position))
	  (#.+bishop+ (position-white-bishops position))
	  (#.+queen+ (position-white-queens position))
	  (#.+king+ (position-white-king position)))
	(case bb-type
	  (#.+pawn+ (position-black-pawns position))
	  (#.+rook+ (position-black-rooks position))
	  (#.+knight+ (position-black-knights position))
	  (#.+bishop+ (position-black-bishops position))
	  (#.+queen+ (position-black-queens position))
	  (#.+king+ (position-black-king position))))))
...
(declaim (ftype (function (position color) mailbox-index) king-square))
(defun king-square (position color)
  (let ((bb (if (= color +white+) (position-white-king position) (position-black-king position))))
...
(defun checkmate-p (position &optional (moves t))
  (and (king-in-check-p position (position-turn position))
       (null moves)))

(defun stalemate-p (position &optional (moves t))
  (and (not (king-in-check-p position (position-turn position)))
       (null moves)))
