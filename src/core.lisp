...
(defun set-piece-at (state square piece)
  (let ((bit (ash 1 square)))
    (case (piece-color piece)
      (:white (case (piece-type piece)
		(:pawn (setf (white-pawns state) (logior (white-pawns state) bit)))
		(:rook (setf (white-rooks state) (logior (white-rooks state) bit)))
		(:knight (setf (white-knights state) (logior (white-knights state) bit)))
		(:bishop (setf (white-bishops state) (logior (white-bishops state) bit)))
		(:queen (setf (white-queens state) (logior (white-queens state) bit)))
		(:king (setf (white-king state) (logior (white-king state) bit)))))
      (:black (case (piece-type piece)
		(:pawn (setf (black-pawns state) (logior (black-pawns state) bit)))
		(:rook (setf (black-rooks state) (logior (black-rooks state) bit)))
		(:knight (setf (black-knights state) (logior (black-knights state) bit)))
		(:bishop (setf (black-bishops state) (logior (black-bishops state) bit)))
		(:queen (setf (black-queens state) (logior (black-queens state) bit)))
		(:king (setf (black-king state) (logior (black-king state) bit)))))
      (otherwise nil)))
